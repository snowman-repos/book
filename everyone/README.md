...
Some things are not even worth learning because they can be so easily referenced. Obscure CSS hacks for instance. On the rare occasion that you run into a problem that can be easily fixed with 30 seconds on a search engine, then why waste precious time committing it to memory?
