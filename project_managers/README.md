...
_Reading time ~ 3 hours, 30 minutes_
...
In order to foster smooth team collaboration, make people sit together. You may think this is trivial or silly or disruptive. People are comfortable where they are, right? Traditional factory floors or print media production companies may be able to arrange people into separate divisions because their work is perhaps isolated from or at least sequential to each other. But as we've already established that can't be the case for web projects, where we have inter-dependent roles working closely together. So avoid having a design team, and a developer team, and a testing team, all sitting in their own areas. Instead gather your own team for each project and make them sit together. Have your office space arranged such that everyone is free to move around. By having the content producers, the developers, the designers, yourself, maybe even the project owners, sitting together, not only are you able to communicate more quickly and directly - you don't have to get up from your desk and walk across the office - but you can also see what the others are doing, better understand their processes, and monitor their progress. A tester can ask questions rather than raising potentially unnecessary bug reports. Designers can more easily check whether an idea is technically feasible when the developer who will implement the design is sitting next to them; likewise a developer might be able to step in if he notices the designer playing with ideas that will be difficult to pull off. Ideally they'll be working closely together to create the design anyway, but it's a lot easier when they're sitting together, and will surely prevent those stereotypical conflicts.
...
The project plan should outline the exact deliverables required for each project. However there may be other optional deliverables that you may wish to ask of your team. An obvious one is documentation, and it's especially important if you have a large or changing team. People need to be able to understand the work of others if they are to contribute to and build on it. Depending on the type of work they're doing, they may need to produce written documents or code comments. Maybe there are tools you can suggest to make this task easier for them. You could set up an online wiki for instance, provide documentation templates, or install plugins for code editors that generate comment blocks automatically.
...
Essentially how it works is everyone has a local code repository on their computer on which they run their own changes. You have the option of having a remote repository on a private server, or on a service like Github. Git will determine how the changes fit together and synchronise everyone's work. When anyone needs the latest copy of the code, they can “pull” it down from the centralised remote repository.
...
Git encourages modularisation. Multiple components worked on in parallel can be isolated into “branches” so that the ongoing development work isn't in conflict; changing one thing won't break another. Branches can signify which code is currently in development, which is ready for testing and which is ready for deployment. It's up to you how your team branch their code. Maybe you would have separate branches for each person to work on independently. Maybe you want separate branches for each feature or feature set. Maybe you'll have separate branches for design work, front-end and back-end code, etc. You'll certainly want separate branches for development, staging, and production. When people are finsished the work on a particular branch, they can “merge” it back into a centralised branch that can then be deployed to a test server.

Version control systems also encourages creativity and experimentation because in your isolated branch you know your work can't negatively impact anything else and that you can always undo your mistakes. It allows people to work offline as it doesn't require constant access to a remote server - people can work on their own machines and commit their work when they do have access. Using a system like git, especially in combination with a service like Github, pretty much ensures that work will never be lost. That's because everyone has a full local copy of the project on their computer, including the project's full change history, which can be used to restore if necessary.
...
A development server is one that your developers deploy to and testers test on. A staging server is one on which you deploy work to be presented to and tested by the project owner. If they approve what they see on the staging server then you can deploy it to the production server, live for the world to access. Obviously you'll want to restrict access to the development and staging servers from both users and search engines by setting up passwords and robots files, respectively. You may ask why you need separate development and staging servers and if the added expense is really worth it. Assuming your team are continuously working on the project while the project owner is reviewing it, you don't want anything to break or change unexpectedly. Perhaps there are reasons why your development server cannot accurately mimic the production environment, in which case a staging server is essential as a sanity check before you deploy. This is especially true when you have multiple projects being developed and tested on one development server, impacting its performance.

A handy way to ensure the server environments are always identical is to use a cloning or imaging system, such as a virtual machine, managed with a tool like Vagrant. Obviously you'll want to customise what server-side software is required depending on the technology stack for each project. 
You may choose to have a physical server located in your office, or to use an online service such as Amazon. An online service may be more expensive in the long term but the overall costs are still fairly low and such services offer the advantages of backups, recovery, redundancy, speed, and security, none of which anyone on your team needs to manage. If the project owner is in another country then a server that's located in your office may be difficult or slow for them to access, making performance issues difficult, if not impossible, for your team to investigate.
